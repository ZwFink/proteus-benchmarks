...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (!std::strcmp(argv[i], "--N") || !std::strcmp(argv[i], "-n")) {
      ...
      if (i + 1 < argc) {
        KernelType = argv[++i];
        if (KernelType != "cuda" && KernelType != "gpu_regtiled") {
  ...
  std::cout << "3mm CUDA-annotation: N=" << N
            << ", trials=" << NumTrials
            << ", verify=" << DoVerify
            << ", kernel=" << KernelType
  ...
  if (KernelType == "gpu_regtiled") {
    cudaRegSharedTiledMatmulLaunch(AD, BD, ED, N);
    cudaRegSharedTiledMatmulLaunch(CD, DD, FD, N);
    cudaRegSharedTiledMatmulLaunch(ED, FD, GD, N);
  } else {
    cudaNontiledMatmulLaunch(AD, BD, ED, N);
    cudaNontiledMatmulLaunch(CD, DD, FD, N);
    cudaNontiledMatmulLaunch(ED, FD, GD, N);
  }
