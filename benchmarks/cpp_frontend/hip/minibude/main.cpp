...
constexpr std::string_view StrFastenKernelTemplate = R"cpp(
{{ include }}
#include <cmath>
#include <cfloat>
...
constexpr size_t MAX_PPWI = {{ max_ppwi }};
constexpr size_t posesPerWI = {{ posesPerWI }};
constexpr size_t nposes = {{ nposes }};
constexpr size_t natlig = {{ natlig }};
...
void printTimings(const Params &params, double millis) {
  ...
  double total_ops = ops_per_wg * (static_cast<double>(params.nposes) / params.posesPerWI);
  double flops = total_ops / runtime;
  double gflops = flops / 1e9;

  double interactions = static_cast<double>(params.nposes) * static_cast<double>(params.natlig) * static_cast<double>(params.natpro);
...
Params loadParameters(const std::vector<std::string> &args) {
  ...
  for (size_t i = 0; i < 6; ++i) {
    ...
    std::copy(std::next(poses.cbegin(), static_cast<std::ptrdiff_t>(i * params.nposes)),
              std::next(poses.cbegin(), static_cast<std::ptrdiff_t>(i * params.nposes + params.nposes)),
...
std::vector<float> runKernel(const Params &params) {
  ...
  gpuErrCheck(gpuMalloc(reinterpret_cast<void **>(&d_protein_x), params.natpro * sizeof(float)));
  gpuErrCheck(gpuMalloc(reinterpret_cast<void **>(&d_protein_y), params.natpro * sizeof(float)));
  gpuErrCheck(gpuMalloc(reinterpret_cast<void **>(&d_protein_z), params.natpro * sizeof(float)));
  gpuErrCheck(gpuMalloc(reinterpret_cast<void **>(&d_protein_type), params.natpro * sizeof(int32_t)));
  ...
  gpuErrCheck(gpuMalloc(reinterpret_cast<void **>(&d_ligand_x), params.natlig * sizeof(float)));
  gpuErrCheck(gpuMalloc(reinterpret_cast<void **>(&d_ligand_y), params.natlig * sizeof(float)));
  gpuErrCheck(gpuMalloc(reinterpret_cast<void **>(&d_ligand_z), params.natlig * sizeof(float)));
  gpuErrCheck(gpuMalloc(reinterpret_cast<void **>(&d_ligand_type), params.natlig * sizeof(int32_t)));
  ...
  gpuErrCheck(gpuMalloc(reinterpret_cast<void **>(&d_ff_hbtype), params.ntypes * sizeof(int32_t)));
  gpuErrCheck(gpuMalloc(reinterpret_cast<void **>(&d_ff_radius), params.ntypes * sizeof(float)));
  gpuErrCheck(gpuMalloc(reinterpret_cast<void **>(&d_ff_hphb), params.ntypes * sizeof(float)));
  gpuErrCheck(gpuMalloc(reinterpret_cast<void **>(&d_ff_elsc), params.ntypes * sizeof(float)));
  ...
  gpuErrCheck(gpuMalloc(reinterpret_cast<void **>(&results), params.nposes * sizeof(float)));

  size_t global = static_cast<size_t>(std::ceil(params.nposes / static_cast<double>(params.posesPerWI)));
  global = static_cast<size_t>(std::ceil(static_cast<double>(global) / params.wgSize));
  ...
  using FastenSig = void(const float *, const float *, const float *,
                         ...
                         const float *, const float *, const float *,
                         float *);
  auto kernel = jitModule.getKernel<FastenSig>("fasten_main");

  // warmup
  kernel.launch({grid.x, grid.y, grid.z},
